...
export const env = {
  ...
  /** Where clients reschedule/cancel (Cal.com's booking pages). */
  calAppUrl: () => (process.env.CALCOM_APP_URL ?? "https://cal.com").replace(/\/$/, ""),
  stripeSecretKey: () => required("STRIPE_SECRET_KEY"),
  /** Billing is optional: the section is hidden unless a Stripe key is configured. */
  stripeEnabled: () => Boolean(process.env.STRIPE_SECRET_KEY),
  /** Change/Cancel links are hidden for sessions starting sooner than this. 0 disables the rule. */
